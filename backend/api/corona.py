...
def add_data(start, end):
    ...
    for item in data[::-1]:
        ...
        # 2020-04-14 -> 20200414
        createdt = "".join(createdt[:10].split("-"))
        date = datetime(int(createdt[:4]), int(
            createdt[4:6]), int(createdt[6:]))
        ...
        if i % 11 == 10:        # 하루 데이터가 끝나는 시점
            ...
            insert_data(date, total_conf, total_death_toll,
                        death_by_age, conf_by_age)
...
def insert_data(date, total_conf, total_death_toll, death_by_age, conf_by_age):
    ...
    try:
        ...
        total_critical_rate = calculate_critical_rate(
            total_conf, total_death_toll, pre_conf, pre_death)
    except:
    ...
    for i in range(9):
        ...
        if i == 3:          # ~ 40세 미만
            ...
            try:
                ...
                first_ciritical_rate = calculate_critical_rate(
                    temp_conf, temp_death, temp_pre_conf, temp_pre_death)
            ...
            try:
                ...
                second_ciritical_rate = calculate_critical_rate(
                    temp_conf, temp_death, temp_pre_conf, temp_pre_death)
            ...
            try:
                ...
                third_ciritical_rate = calculate_critical_rate(
                    temp_conf, temp_death, temp_pre_conf, temp_pre_death)
